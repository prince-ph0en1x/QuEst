...
def test_sequence_qasm(params):
    ...
    for q in range(NUM_QUBIT):
        k1.ry(q + NUM_QUBIT, params[q + NUM_QUBIT])
...
function for the aforementioned paper
"""
...
if __name__ == '__main__':
    ...
    x0 = [0 for _ in range(NUM_QUBIT)]
    res = minimize(minimize_func, x0, args=(pre_hst, post_hst), method='Powell', tol=1e-6)
    print(['{:.2f}'.format(i) for i in COSTS])
    print('Params: ', res.x)
